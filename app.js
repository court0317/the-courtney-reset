...
state={checks:{},water:{},hydration:{},mealDone:{},weights:{},notes:{},shopping:{},checkins:{},celebrated:{},dayMeals:{},leftovers:[],customRecipes:[],...state};
...
function customRecipeById(id){return state.customRecipes.find(x=>x.id===id)}
function recipeTimeText(r){const total=(+r.prep||0)+(+r.cook||0);return total?`${total} min`:''}
function renderMyRecipes(){
 const root=document.getElementById('myRecipeList');if(!root)return;
 ...
 let list=[...state.customRecipes].filter(r=>(filter==='all'||r.type===filter)&&(!q||[r.name,r.type,...(r.tags||[]),...(r.ingredients||[])].join(' ').toLowerCase().includes(q)));
 list.sort((a,b)=>(b.favorite?1:0)-(a.favorite?1:0)||String(b.updatedAt||'').localeCompare(String(a.updatedAt||'')));
 root.innerHTML=list.length?list.map(r=>`<article class="my-recipe-card" data-view-recipe="${r.id}">
   <div class="my-recipe-photo">${r.photo?`<img src="${r.photo}" alt="${escapeHtml(r.name)}">`:`<span>${r.icon||'🍽️'}</span>`}</div>
   <div class="my-recipe-copy"><div class="my-recipe-title"><div><small>${escapeHtml(r.type)}</small><h4>${escapeHtml(r.name)}</h4></div><button class="recipe-favourite ${r.favorite?'active':''}" data-favourite-recipe="${r.id}" aria-label="Favourite">${r.favorite?'♥':'♡'}</button></div>
   <div class="recipe-meta-row">${r.cal?`<span>${r.cal} kcal</span>`:''}${r.protein?`<span>${r.protein}g protein</span>`:''}${recipeTimeText(r)?`<span>${recipeTimeText(r)}</span>`:''}</div>
   <div class="my-recipe-actions"><button class="primary" data-add-custom-meal="${r.id}">Add to today</button><button class="outline" data-edit-recipe="${r.id}">Edit</button><button class="text-btn recipe-delete" data-delete-recipe="${r.id}">Delete</button></div></div>
 </article>`).join(''):'<div class="recipe-empty"><span>📖</span><b>Your recipe book is ready</b><p>Add your first meal and it will appear here.</p><button class="primary" data-open-recipe-editor>+ Add your first recipe</button></div>';
 root.querySelectorAll('[data-view-recipe]').forEach(el=>el.onclick=e=>{if(e.target.closest('button'))return;openRecipeView(el.dataset.viewRecipe)});
 root.querySelectorAll('[data-open-recipe-editor]').forEach(x=>x.onclick=()=>openRecipeEditor());
 root.querySelectorAll('[data-favourite-recipe]').forEach(x=>x.onclick=e=>{e.stopPropagation();const r=customRecipeById(x.dataset.favouriteRecipe);r.favorite=!r.favorite;r.updatedAt=new Date().toISOString();save()});
 root.querySelectorAll('[data-edit-recipe]').forEach(x=>x.onclick=e=>{e.stopPropagation();openRecipeEditor(x.dataset.editRecipe)});
 root.querySelectorAll('[data-delete-recipe]').forEach(x=>x.onclick=e=>{e.stopPropagation();const r=customRecipeById(x.dataset.deleteRecipe);if(confirm(`Delete “${r.name}”?`)){state.customRecipes=state.customRecipes.filter(y=>y.id!==r.id);save()}});
 root.querySelectorAll('[data-add-custom-meal]').forEach(x=>x.onclick=e=>{e.stopPropagation();chooseCustomRecipeMeal(x.dataset.addCustomMeal)});
}
function resetRecipeForm(){
 recipeEditorForm.reset();recipeEditId.value='';recipeServes.value=1;recipeType.value='Dinner';recipePhoto.value='';recipePhotoPreview.hidden=true;recipePhotoPreview.removeAttribute('src');recipePhotoEmpty.hidden=false;recipeEditorTitle.textContent='Add a recipe';recipeEditorModal.dataset.photo='';
}
function openRecipeEditor(id=''){
 resetRecipeForm();const r=id?customRecipeById(id):null;
 if(r){recipeEditorTitle.textContent='Edit recipe';recipeEditId.value=r.id;recipeName.value=r.name||'';recipeType.value=r.type||'Dinner';recipeServes.value=r.serves||1;recipeCalories.value=r.cal||'';recipeProtein.value=r.protein||'';recipeCarbs.value=r.carbs||'';recipeFat.value=r.fat||'';recipePrep.value=r.prep||'';recipeCook.value=r.cook||'';recipeIngredients.value=(r.ingredients||[]).join('\n');recipeMethod.value=r.method||'';recipeTags.value=(r.tags||[]).join(', ');recipeEditorModal.dataset.photo=r.photo||'';if(r.photo){recipePhotoPreview.src=r.photo;recipePhotoPreview.hidden=false;recipePhotoEmpty.hidden=true}}
...
async function resizeRecipePhoto(file){
 ...
 const max=900,scale=Math.min(1,max/Math.max(img.width,img.height));const canvas=document.createElement('canvas');canvas.width=Math.round(img.width*scale);canvas.height=Math.round(img.height*scale);canvas.getContext('2d').drawImage(img,0,0,canvas.width,canvas.height);return canvas.toDataURL('image/jpeg',.76)
}
function chooseCustomRecipeMeal(id){const r=customRecipeById(id);if(!r)return;const options=['Breakfast','Lunch','Dinner','Snack','Extra'];let suggested=options.includes(r.type)?r.type:'Dinner';const type=prompt('Add this recipe to which meal?\nBreakfast, Lunch, Dinner, Snack or Extra',suggested);if(!type)return;const clean=options.find(x=>x.toLowerCase()===type.trim().toLowerCase());if(!clean)return alert('Please enter Breakfast, Lunch, Dinner, Snack or Extra.');setMeal(clean,{...cloneMeal(r),type:clean,custom:true});alert(`${r.name} added to ${clean}.`)}
function openRecipeView(id){const r=customRecipeById(id);if(!r)return;recipeViewContent.innerHTML=`${r.photo?`<img class="recipe-view-photo" src="${r.photo}" alt="${escapeHtml(r.name)}">`:''}<span class="eyebrow">${escapeHtml(r.type)}</span><h2>${escapeHtml(r.name)}</h2><div class="recipe-view-stats">${r.serves?`<span><small>Serves</small><b>${r.serves}</b></span>`:''}${r.cal?`<span><small>Calories</small><b>${r.cal}</b></span>`:''}${r.protein?`<span><small>Protein</small><b>${r.protein}g</b></span>`:''}${recipeTimeText(r)?`<span><small>Total time</small><b>${recipeTimeText(r)}</b></span>`:''}</div><h3>Ingredients</h3><ul class="recipe-view-ingredients">${(r.ingredients||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul><h3>Method</h3><div class="recipe-view-method">${escapeHtml(r.method||'').replace(/\n/g,'<br>')}</div>${(r.tags||[]).length?`<div class="recipe-view-tags">${r.tags.map(x=>`<span>${escapeHtml(x)}</span>`).join('')}</div>`:''}<div class="button-row"><button class="primary" data-view-add="${r.id}">Add to today</button><button class="outline" data-view-edit="${r.id}">Edit recipe</button></div>`;recipeViewContent.querySelector('[data-view-add]').onclick=()=>{recipeViewModal.close();chooseCustomRecipeMeal(r.id)};recipeViewContent.querySelector('[data-view-edit]').onclick=()=>{recipeViewModal.close();openRecipeEditor(r.id)};recipeViewModal.showModal()}
...
addRecipeBtn.onclick=()=>openRecipeEditor();cancelRecipeBtn.onclick=()=>recipeEditorModal.close();recipeSearch.oninput=renderMyRecipes;recipeFilter.onchange=renderMyRecipes;recipePhoto.onchange=async()=>{try{const data=await resizeRecipePhoto(recipePhoto.files[0]);recipeEditorModal.dataset.photo=data;recipePhotoPreview.src=data;recipePhotoPreview.hidden=false;recipePhotoEmpty.hidden=true}catch(e){alert(e.message)}};recipeEditorForm.onsubmit=e=>{e.preventDefault();const id=recipeEditId.value||`custom-${Date.now()}-${Math.random().toString(36).slice(2,7)}`;const old=customRecipeById(id)||{};const item={...old,id,name:recipeName.value.trim(),type:recipeType.value,icon:'🍽️',effort:'easy',category:'custom',serves:+recipeServes.value||1,cal:+recipeCalories.value||0,protein:+recipeProtein.value||0,carbs:+recipeCarbs.value||0,fat:+recipeFat.value||0,prep:+recipePrep.value||0,cook:+recipeCook.value||0,ingredients:recipeIngredients.value.split(/\n/).map(x=>x.trim()).filter(Boolean),method:recipeMethod.value.trim(),tags:recipeTags.value.split(',').map(x=>x.trim()).filter(Boolean),photo:recipeEditorModal.dataset.photo||'',custom:true,weekday:true,updatedAt:new Date().toISOString(),createdAt:old.createdAt||new Date().toISOString()};if(!item.name||!item.ingredients.length||!item.method)return alert('Please add a meal name, ingredients and method.');const existing=state.customRecipes.findIndex(x=>x.id===id);if(existing>=0)state.customRecipes[existing]=item;else state.customRecipes.unshift(item);recipeEditorModal.close();save();window.RootedCloud?.syncNow?.();};
...
if('serviceWorker'in navigator)addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=330recipes').catch(()=>{}));render();
