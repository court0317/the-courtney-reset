(() => {
  ...
  function setImportFeedback(statusMessage = '', errorMessage = '') {
    ...
    const error = $('recipeImportError');
    if (status) status.textContent = statusMessage;
    if (error) error.textContent = errorMessage;
  }

  function setEditorFeedback(message = '') {
    const status = $('recipeEditorDraftStatus');
  ...
  function addToShoppingList(id) {
    const r = recipes.find(x => x.id === id); if (!r) return;
    const items = (window.FlourishBloomAppState?.shoppingList || []);
    const additions = [...(r.ingredients || []), ...(r.tags || [])];
    additions.forEach(item => {
      const text = String(item || '').trim();
      if (text && !items.includes(text)) items.push(text);
    });
    if (window.FlourishBloomAppState) {
      window.FlourishBloomAppState.shoppingList = items;
